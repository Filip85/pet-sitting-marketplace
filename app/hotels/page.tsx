...
export default async function HotelsPage({
  ...
  let externalDebug: Awaited<ReturnType<typeof fetchExternalPetHotelsByCityDetailed>>['debug'] | null = null
  ...
  if (source === 'external') {
    if (city) {
      try {
        ...
        externalDebug = externalResult.debug
      } catch {
        externalError = 'External provider is temporarily unavailable. Please try again.'
      }
    }
  } else {
    try {
      ...
      internalError = 'Registered hotels are temporarily unavailable. Please try again.'
    }
  }
  ...
  return (
    <PageContainer className="py-10 sm:py-12">
      <div className="rounded-3xl bg-gradient-to-br from-emerald-50 via-cyan-50 to-white border border-emerald-100 p-8 sm:p-10 mb-8 shadow-sm">
        <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Pet Hotels</h1>
        <p className="text-sm text-gray-500 mt-2">Switch between registered hotels and external Internet listings.</p>
      </div>
      ...
      {source === 'external' ? (
        <>
          <div className="mb-5 text-sm text-gray-500">
            {externalHotels.length} {externalHotels.length === 1 ? 'internet hotel' : 'internet hotels'} found
          </div>

          {!city ? (
            <div className="rounded-3xl bg-white border border-gray-100 px-6 py-10 text-center">
              <p className="text-gray-700 font-semibold mb-1">Enter a city to load Internet hotels.</p>
              <p className="text-gray-500 text-sm">This view shows only external pet-hotel listings.</p>
            </div>
          ) : externalError ? (
            <div className="rounded-3xl bg-red-50 border border-red-100 px-6 py-10 text-center">
              <p className="text-red-700 font-semibold mb-1">Could not load Internet hotels.</p>
              <p className="text-red-500 text-sm">{externalError}</p>
            </div>
          ) : externalHotels.length === 0 ? (
            <div className="rounded-3xl bg-white border border-gray-100 px-6 py-10 text-center">
              <p className="text-gray-700 font-semibold mb-1">No Internet hotels found for that city.</p>
              <p className="text-gray-500 text-sm">Try another city name.</p>
            </div>
          ) : (
        ...
        <div className="rounded-3xl bg-red-50 border border-red-100 px-6 py-10 text-center">
          <p className="text-red-700 font-semibold mb-1">Could not load registered hotels.</p>
          <p className="text-red-500 text-sm">{internalError}</p>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="mb-5 text-sm text-gray-500">
            {internalHotels.length} {internalHotels.length === 1 ? 'registered hotel' : 'registered hotels'} found
          </div>

          {city ? (
            <div className="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
              Showing registered pet hotels for {city}.
            </div>
          ) : null}

          <SitterGrid
            sitters={internalHotels}
            cardBadgeLabel="Registered hotel"
            emptyTitle="No registered hotels available yet"
            emptyMessage={city ? 'Try another city name or switch back to external hotels.' : 'Add a city to filter registered hotels, or switch to external listings.'}
          />
        </div>
      )}
    </PageContainer>
