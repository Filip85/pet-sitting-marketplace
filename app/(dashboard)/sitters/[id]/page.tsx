  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        services_offered, can_host_at_home,
...
export default async function SitterProfilePage({
  ...
  const { id } = await params
  const [sitter, t, tSvc] = await Promise.all([fetchSitter(id), getTranslations('Sitters'), getTranslations('ServiceLabels')])
        ...
        <div className="p-8">
          <div className="flex items-start gap-6 mb-8">
            ...
            <div className="shrink-0 text-right">
              ...
              <p className="text-sm text-gray-400 mt-1">{t('perDay')}</p>
            </div>
          </div>
          ...
          <div className="mb-6">
            <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-3">{t('about')}</h2>
          ...
          <div className="mb-6">
            <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">{t('details')}</h2>
            <dl className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-gray-50 rounded-xl px-4 py-3">
                <dt className="text-xs text-gray-400 mb-1">{t('experience')}</dt>
                <dd className="text-sm font-semibold text-gray-900">
                  {sitter.years_of_experience != null && sitter.years_of_experience > 0
                    ? t('yearsExp', { count: sitter.years_of_experience })
                    : t('notSpecified')}
                </dd>
              </div>
              <div className="bg-gray-50 rounded-xl px-4 py-3">
                <dt className="text-xs text-gray-400 mb-1">{t('canHost')}</dt>
                <dd className="text-sm font-semibold text-gray-900">
                  {sitter.can_host_at_home ? t('yes') : t('no')}
                </dd>
              </div>
              <div className="bg-gray-50 rounded-xl px-4 py-3">
                <dt className="text-xs text-gray-400 mb-1">{t('rate')}</dt>
                <dd className="text-sm font-semibold text-gray-900">{formattedPrice} / {t('perDay')}</dd>
              </div>
            </dl>
          </div>
          ...
          <div className="border-t border-gray-100 mb-6" />
          <div className="mb-8">
            <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-3">{t('servicesTitle')}</h2>
            {serviceIds.length > 0 ? (
              <ul className="flex flex-wrap gap-2">
                {serviceIds.map((sid) => {
                  ...
                  return (
                    ...
                    >
                      {s?.icon ?? '•'} {s ? tSvc(s.id as Parameters<typeof tSvc>[0]) : sid}
                    </li>
                  )
                })}
              </ul>
          ...
          {!user ? (
            ...
            >
              {t('loginToBook', { price: formattedPrice })}
            </Link>
          ) : canRequestBooking ? (
            <Link
              ...
              className="w-full inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold py-3 px-6 rounded-xl transition-colors text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
            >
              {t('requestBooking', { price: formattedPrice })}
            </Link>
          ) : (
