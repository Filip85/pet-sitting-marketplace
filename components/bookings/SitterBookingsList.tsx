...
export function SitterBookingsList({ bookings }: { bookings: SitterBookingListItem[] }) {
  if (bookings.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center">
        <p className="text-3xl mb-3">📥</p>
        <h3 className="text-gray-900 font-semibold">No incoming requests</h3>
        <p className="text-sm text-gray-400 mt-1">When owners request a booking, it will appear here.</p>
      </div>
    )
  }
  ...
  return (
    <ul className="space-y-3">
      {bookings.map((b) => {
        ...
        const petTitle = b.pet
          ? `${TYPE_EMOJI[b.pet.type] ?? '🐾'} ${b.pet.name}`
          : '🐾 Pet'
