...
export async function SitterCard({ sitter, badgeLabel }: SitterCardProps) {
  if (!sitter.profile) return null
  ...
  const shortBio = bio?.trim() || t('noBioCard')
      ...
      <div className="flex flex-col flex-1 p-7">
        <div className="flex items-start gap-4 mb-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-lg font-bold text-gray-900 leading-snug">{fullName}</h3>
            <p className="text-sm text-gray-500 mt-1">{city ?? t('locationNotSet')}</p>
          </div>

          <div className="shrink-0 text-right">
            <p className="text-xl font-bold text-blue-600 leading-none">{formattedPrice}</p>
            <p className="text-xs text-gray-400 mt-1">/ {t('perDay')}</p>
          </div>
        </div>

        <p className="text-sm text-gray-600 leading-relaxed line-clamp-3 mb-6">{shortBio}</p>
